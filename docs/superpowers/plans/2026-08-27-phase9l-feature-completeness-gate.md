...
- [x] RED `tests/no-fabrication.test.mjs`: across every tracked `docs/**/*.md` and `README.md`, a **capacity or performance figure** — a number immediately followed by `req/s`, `requests/s`, `rps`, `MB/s`, `concurrent requests`, or a latency figure presented as a *result* (`p50`, `p95`, `p99`, `throughput`, `measured`) — must be on a line or in a table row that also carries a `transcript:` reference. An unsourced benchmark number is the easiest thing in this repository to fabricate, so it is mechanically forbidden. — *Widened in two ways the plan's wording did not anticipate, both because the real corpus demanded it. **Scope:** "every tracked `docs/**/*.md` and `README.md`" silently omits `WORK-HANDOFF.md`, `packaging/README.md`, and `scripts/fuzz/corpus/regression/README.md` — all tracked markdown, and the handoff holds more measurements than any other file. The scope is now *every tracked `.md` except `docs/transcripts/**`*, and a test asserts the exclusion list contains nothing but transcripts, because a scope that omits a document by accident is indistinguishable from a scan that finds nothing. **Source form:** a `transcript:` reference alone would have refused a dozen honest figures whose provenance is the script that produced them (`scripts/fuzz/host-baseline.mjs`, `scripts/soak-lib.mjs`); any citation or producing-file path counts.*
- [x] **Citation scope is the block, not the line.** 9I's load table puts five levels' figures in five rows and its transcript path in the sentence above them. Demanding a citation per row would have forced one path repeated five times — which nobody maintains, so it drifts — so a table inherits the paragraph immediately above it. **Two real documents were unsourced under that rule and were fixed by adding the citation, not by loosening it:** the handoff's streaming-TTFB paragraph and the resilience report's heap-floor paragraph, both of which quote figures whose transcript sits in a different block.
- [x] RED same file: the figure rule must **not** trip on a configured bound, which is a design decision rather than a measurement. Timeouts, buffer caps, retention limits, and sampling intervals (`64 KiB`, `250 ms`, `±60s`, `every 15 s`, `32 KiB`) are exempt, and the test pins several real examples from the 9B, 9F, and 9I plans as must-pass cases. A rule that cried wolf on every plan document would be turned off within a week, which is worse than not having it. — *Six real bound lines pinned as must-pass. **The exemption is deliberately asymmetric:** a rate figure (`req/s`, `MB/s`) bypasses the bound vocabulary entirely, because `40 req/s` is a measurement however the sentence is worded — otherwise "the default throughput limit is 5,000 req/s" would launder a fabricated number through one exempting word, and a test asserts that exact string still fails.*
- [x] RED same file: no tracked document **claims** `zeroize`/`zeroise`/`zeroization`, `memory wiped`, `securely erased`, `reproducible build`, `tamper-proof`, `unhackable`, `military-grade`, `bank-grade`, or `100% secure`, under the negation-aware rule above. The test names the offending file, line, and matched term. — *32 mentions in the real corpus, up from the plan's measured twelve; **all 32 pass**, and a test asserts the count stays ≥ 12 so the scan cannot go quiet by the corpus emptying out.*
- [x] **Negation is sentence-scoped, not line-scoped, and that was a correctness fix rather than a refinement.** Line-scoping accepted `The proxy is not enabled by default. Secrets are zeroized from memory.` — one honest refusal laundering one real claim on the same line. A test pins that exact string as a finding. Two further carve-outs, each forced by a document that would otherwise have had to be damaged to satisfy the rule: an **inline code span or quoted phrase** is a mention, not an assertion (§25.6 writes `— not *"you will never be charged"*` in order to forbid it; the Phase 2 design writes `` `linux arm64` `` naming the runtime it measured), and a **mutation record** is evidence *for* the policy (9K's plan records "the tarball verdict reworded to claim a reproducible build (2 red)" — the same trap Task 1 hit with a `PASS`-shaped example in prose, where satisfying the rule would mean deleting the record of the defect it guards against).
- [x] RED same file: no document claims support for a platform whose 9J matrix row is not `PASS`, and no document claims a client works whose 9H matrix row is not `PASS` — asserted by cross-reading both matrices, so the README cannot drift ahead of the evidence. — *Both lists are **read from the matrices** through 9J's own `parseMatrix`, never hardcoded: a second copy of the matrix would drift, and the copy that drifted would be the one deciding whether the README is honest. A vacuity test asserts both lists are non-empty **and** names `Windows x64` and `antigravity` as withheld — plus, in the other direction, that `hermes` is **not** in the withheld list. That reverse assertion caught a real parser defect: each client section also carries a transcript-inventory table, and reading its prose second column as a status marked `hermes` unverified despite 17 of 17, which would have silently suppressed every honest sentence recording that Hermes was driven for real.*
- [x] RED same file: no `PASS` anywhere in any tracked matrix or report lacks an evidence reference (a repo-wide sweep, catching a matrix a future phase adds without wiring it into a gate). — *Run over **all** tracked markdown including transcripts — a transcript is excluded from the figure rule because it is the source, but a `PASS` is a verdict wherever it appears. The sweep reads each row's **last** status cell, so a legend row explaining the vocabulary is not mistaken for a claim, and it accepts the bare backticked-path citation form the 9H client pages use as well as `kind:path`. **Every one of the citations it finds is resolved through `scripts/evidence.mjs`, not pattern-matched** — 198 positive rows carrying 210 citations, all resolving.*
- [x] Verify RED. — *Three mutations planted in `README.md` in one edit — `40,000 req/s`, `memory is zeroized after every request`, and `Windows x64 is fully supported` — went **3 red** across exactly the three intended rules, then reverted.*
- [x] GREEN: correct any document the scan catches. If a claim cannot be evidenced, the claim is removed or downgraded — never the test loosened. — *Two documents corrected by adding the missing citation (above). No claim needed removing, and no rule was loosened to accommodate one.*
- [x] Verify: `node --test tests/no-fabrication.test.mjs` exits 0 against **every** existing document, including the Phase 9 specs and plans themselves. — ***20/20.*** *`matrix-integrity` 9/9, `resilience-report` 24/24, `platform-matrix` 12/12, `feature-gate-integrity` 29/29, `client-docs` 6/6, `evidence` 18/18 — 96/96 together. `client-docs`, `platform-gate` and `pack` (39/39) all still pass after the README edit.*
- [x] Commit — `test: forbid unevidenced Bayz claims in documentation`
...
- [x] Amend Task 4: the anti-fabrication scan forbids any document claiming BAYZ guarantees the operator will never be charged. The honest claim, per spec §25.6, is that BAYZ never selected a paid model without metadata saying it was free. The test asserts the strong form is absent and the honest form is present in the README. — *A rule of its own, because it is a claim about **money** rather than about capability. Two shapes in the corpus are honest and neither is the forbidden claim: §25.1's "must never be charged **because BAYZ silently chose a paid route**" attributes the harm to the one cause BAYZ controls, and both the spec and this plan quote the strong form in order to forbid it. The README gained the §25.6 boundary and the narrow claim, written from `packages/router/src/selection.ts` and `router.ts` rather than from the spec's wording: an unclassified model is **not** free, and the classification is re-checked on every failover attempt.*
