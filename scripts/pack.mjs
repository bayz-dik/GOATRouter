...
const PKG_NAME = ${JSON.stringify(pkgName)};
...
if (args.includes("--check-update")) {
  ...
  const { mainCheckUpdate } = await import("./update-cli.mjs");
  process.exitCode = await mainCheckUpdate();
  process.exit(process.exitCode);
}

if (args[0] === "update") {
  // Release-based self-update. Talked to in docs/operations.md.
  const { mainUpdate } = await import("./update-cli.mjs");
  process.exitCode = await mainUpdate();
  process.exit(process.exitCode);
}
...
if (args.includes("--help") || args.includes("-h") || args[0] === "help") {
  process.stdout.write(
    [
      "bayz " + VERSION,
      "",
      "Usage: bayz [command] [--version] [--help]",
      "",
      "Commands:",
      "  update          check for and install the latest GitHub release",
      "  --check-update  report whether an update is available (no install)",
      "",
...
// Resolve the installed package root so update can find its own install prefix.
// bayz.mjs lives at <pkg>/dist/bayz.mjs, so the package root is one level up.
