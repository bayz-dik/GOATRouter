...
**A consequence for the amendment below, recorded rather than worked around:** `scripts/proxy-ux-smoke.mjs` prints `ok <label>` with no number, so `smoke:proxy-ux#N` cannot resolve today. The free-only routing row therefore cannot reach `PASS` by citation until that script is numbered. Task 2 addresses it; a test pins the current state so the day the script gains numbering, this note becomes stale loudly rather than quietly.
