...
/**
 * Bundle the operator control plane separately from the server.
 *
 * control.mjs (with lifecycle-lib, doctor-lib, backup-lib, tui, and the
 * data-dir resolver inlined) is the `bayz` command surface. It is a *different*
 * process from the daemon: it spawns the daemon (dist/server.mjs), checks its
 * health, opens the TUI, and drives doctor/backup against the data dir. It must
 * not open SQLite or bind a listener at import time, so it is bundled from its
 * own entry, exactly as the update CLI already is.
 */
function bundleControl({ root, outFile }) {
  const result = buildSync({
    entryPoints: [join(root, "scripts/control.mjs")],
    bundle: true,
    platform: "node",
    target: "node24",
    format: "esm",
  ...
  if (result.errors.length > 0) {
    throw new Error(`control-plane bundling failed: ${result.errors.map((entry) => entry.text).join("; ")}`);
  }
  return result;
}

/**
 * Bundle the release-based update CLI separately from the server.
 *
 * The server entry starts the runtime and opens SQLite at import time, so the
 * `bayz` bin must not import it for `update`/`--check-update` — those commands
 * must work against an installed package without binding a listener. The update
 * flow is self-contained (talks to the GitHub Releases feed, verifies a
 * checksum, reinstalls the verified tarball).
 */
function bundleUpdateCli({ root, outFile }) {
  const result = buildSync({
    entryPoints: [join(root, "apps/server/src/update-cli.ts")],
    bundle: true,
    platform: "node",
    target: "node24",
    format: "esm",
  ...
  if (result.errors.length > 0) {
    throw new Error(`update-cli bundling failed: ${result.errors.map((entry) => entry.text).join("; ")}`);
  }
  return result;
}
 ...
 */
function binSource({ version, pkgName }) {
  return `#!/usr/bin/env node
// GOAT ROUTER ${version} — generated by scripts/pack.mjs. Do not edit.
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
// The operator control plane (daemon lifecycle + TUI + doctor/backup) is bundled
// into dist/control.mjs; this entry imports it and dispatches. It never imports
// the server bundle, so asking for --version or --help creates no data directory
// and binds no listener.
const pkgRoot = join(dirname(fileURLToPath(import.meta.url)), "..");
process.env.BAYZ_UPDATE_PACKAGE_ROOT = pkgRoot;
process.env.BAYZ_UPDATE_CLI = "1";
...
const { cli } = await import("./control.mjs");
await cli();
`;
}
...
export function buildArtifact({ root = REPO_ROOT, outDir } = {}) {
  ...
  const controlBundle = join(staging, "control.mjs");
  bundleControl({ root, outFile: controlBundle });
  const updateBundle = join(staging, "update-cli.mjs");
  bundleUpdateCli({ root, outFile: updateBundle });
  ...
  const entries = [
    ...
    { name: "package/dist/control.mjs", content: readFileSync(controlBundle) },
    { name: "package/dist/update-cli.mjs", content: readFileSync(updateBundle) },
    { name: "package/dist/bayz.mjs", content: binSource({ version, pkgName: ARTIFACT_NAME }), mode: 0o755 },
