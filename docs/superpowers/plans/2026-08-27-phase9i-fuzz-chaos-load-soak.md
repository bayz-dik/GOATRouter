...
- [x] For each of the thirteen targets: 5,000 iterations at a pinned seed; assert every thrown error is an instance of the owning package's error type with a fixed code from the existing vocabulary; assert no `RangeError`, `TypeError`, `ERR_INTERNAL_ASSERTION`, or `ERR_OUT_OF_RANGE` escapes; assert no iteration exceeds a 250 ms budget (a hang is a DoS, so a slow input is a failure with the input recorded); assert process RSS growth across the 5,000 iterations stays under 64 MiB. — All thirteen at 5,000, **39/39 checks**, three consecutive clean full runs. `scripts/fuzz/targets/shared.mjs` holds the shared contract: `expectBayzError` names an engine error separately from a stray custom error (different diagnosis, different report line), and `globalStateSnapshot`/`assertGlobalStateUnchanged` watch `Object.prototype` — a target that fuzzes prototype-pollution shapes and never checks that would miss the exact defect it was aiming at. RSS growth on this device: `sse` 41.5 MiB, `socks5` 22.3, `tool-args` 18.7, `authorization` 10.8, `api-schema` 5.4, every other target under 1 MiB.
- [x] `authorization` target additionally: a 1 MiB bearer value, a bearer with embedded CR/LF, a bearer of 10,000 spaces, and a bearer that is valid for a *revoked* identity all end in `401` with no distinguishable timing class (bounded, documented as indicative). — Real `openSecretStorage` with real envelope crypto and a real identity manager, no stub. **`revoke` rather than `delete`** for the revoked case: `delete` removes the stored key so the comparison never happens, while `revoke` leaves the key intact and only the identity's usable state says no — the harder branch, and the one the plan means. Timing is measured and reported, deliberately **not asserted**: `verifyKey` shape-checks before hashing (so a 1 MiB bearer does not cost one SHA-256 per registered client), which makes a malformed bearer *legitimately* faster — measured 1.5 µs median against 6,017 µs for a valid key. Claiming an indistinguishable timing class on a shared Android device under proot would be a fake security property; the medians are on the record instead.
- [x] `sse` target additionally: interleaved partial frames, a frame split mid-UTF-8-sequence, a 64 KiB line, a 3 MiB total stream, `[DONE]` inside a JSON string, and a stream of pure `\r` bytes — each ends in a bounded outcome per 9B Task 1. — All six, plus assertions on the *outcome* rather than mere survival: a quoted `[DONE]` must leave the reader alive with exactly two frames, content after a real `[DONE]` must be refused, and a 3 MiB stream must hit `MAX_SSE_TOTAL_BYTES`. Generated inputs are **descriptors, not buffers** — the plan wants a 3 MiB stream while the harness caps a generated input at 1 MiB, and both constraints are real, so `run` materialises the bytes from a recipe.
- [x] `migration` target additionally: a database whose `user_version` is any value 0–255 including beyond the current head opens or fails closed, and **never silently downgrades or half-applies** — assert the schema after the attempt is either the original or the head, never in between. — `user_version` sweeps 0–255 by iteration index (not sampled, so every value is visited) plus `-1`, `256`, `1000`, `2**31-1`. The full table/index/trigger set is captured before and after and compared against a reference head shape; a refusal must additionally have changed **nothing**, and an acceptance must be exactly at head.
- [x] `storage-envelope` target additionally: a bit-flip in every byte position of a short envelope fails closed with `master_key_invalid` or `storage_unavailable`, never returning plaintext. — Flip offset and bit are **swept from the iteration index**, so all ~130 byte positions × 8 bits are covered rather than left to 5,000 random draws. `secret_corrupt` is accepted alongside the plan's two codes, read from `assertEnvelopeShape`: a structurally broken envelope reports that, and asserting only the plan's two would have made a correct implementation look broken. A `roundtrip` case proves the honest path still works — a target that only proved failure would pass against an implementation that always failed.
- [x] `scripts/fuzz-run.mjs` runs every target, prints `ok N` / `FAIL N` per target so a smoke citation can reference `smoke:fuzz#N`, writes any failing input to `scripts/fuzz/corpus/regression/` (credential-scanned first), and exits non-zero on any failure. — Relaunches under `tsx` and imports the real TypeScript sources, because fuzzing a compiled copy would fuzz whatever was last built. Check numbers are contractual, append-only, as in `client-conformance.mjs`. The credential scan is the harness's, which refuses to even *record* a failure whose input or error text is credential-shaped.
- [x] Verify: `node scripts/fuzz-run.mjs` exits 0. — **39/39, `fuzz: PASS`**, three consecutive runs.
- [x] Commit — `test: add Bayz boundary fuzz targets`

**One host-level finding that changes how this phase reads timings.** The `socks5` target reported iterations at 8.5 s and 72 s against the 250 ms budget. Before calling that a hang I wrote `scripts/fuzz/host-baseline.mjs`, which runs a bare `connect` + `setTimeout(60)` + `destroy` loop with **no BAYZ code in it**, and reproduced the same shape: p50 63 ms with individual iterations at 8.2 s, 88 s and 184 s, while load average was 0.12, four descriptors were open, and 3,905 MiB of swap was in use. That is diagnosis **D — Termux/proot/futex host failure**, not a BAYZ defect, and it is intermittent (a later run showed max 68 ms). `timer.unref()` was separately ruled out: ref'd and unref'd 60 ms timers both delivered 60–63 ms. Consequence, recorded in the target: a wall-clock budget is a valid DoS signal for CPU-bound boundaries and **not** for socket-bound ones on this host, so `socks5` asserts BAYZ's own deadline fired — the returned `ProxyError("timeout")` — and carries a widened budget citing that file.

**`sse` RSS was 147 MiB, and the bound was not moved.** Chased instead: growth *decelerated* across 1,250 → 10,000 iterations (67.6, 34.6, 50.1, 5.8 MiB, plateauing near 220 MiB) with heap growth 0.4/−1.1/0.0/0.0 MiB and `arrayBuffers` growth 0.0 MiB throughout; first half +65.1 MiB against second half +40.9 MiB, the opposite of a leak's signature. Isolated: building a 64 KiB buffer and discarding it costs 6.5 MiB over 600 iterations, pushing it through a real `SseLineReader` costs 65.1 MiB, and a shared `TextDecoder`, a fresh one, and `Buffer.toString("utf8")` each cost ~0 MiB. So allocator retention of large short-lived strings, not a leak. The fix was to stop re-proving one byte cap hundreds of times — `overflow` had been drawn 481 times and `long-line` 612 — since a cap is proved at, below and above the boundary. Both now run on a deterministic sparse schedule (ten each per 5,000) and growth measures **41.5 MiB**, under the plan's bound with the bound intact.

**A mutation that passed twice, and the oracle it forced.** Replacing `parseChatRequest`'s `isPlainObject` check and unknown-key loop with a bare `typeof === "object"` test left `api-schema` green — twice. The reason is subtle and worth keeping: the parser builds a fresh `ChatRequest` copying only known fields, so an undocumented key that slips past validation is simply *absent* from the output and invisible to any output-shape assertion. The target now carries a `mustReject(body)` oracle that reads the **input** and decides independently whether acceptance is permitted, with the documented key sets mirrored deliberately rather than imported — importing `ALLOWED_KEYS` would make the assertion agree with any regression by construction. The same mutation then failed at iteration 31 on `{"prototype":1}`, and was reverted byte-identical.

**Five target-side defects found and fixed, none of them BAYZ bugs:** `deriveProfile` takes `grantedScopes` as a `ReadonlySet` (400/400 false failures); `assertModelPattern` reports `invalid_route_config`, not `invalid_model` (62 false failures); SOCKS5 reply codes 0x07/0x08 legitimately map to `unsupported_operation`; `IdentityManager` exposes `createIdentity`, not `create`; and a generator that reached into `choices[0]` *after* replacing `choices` threw inside `generate`, aborting a run rather than fuzzing. Each would have been reported as a product defect by a less careful target.

**The `migration` target's first finding was also mine, not BAYZ's.** It reported a half-applied schema at iteration 48 — but nothing in BAYZ calls `runMigrations` on an unverified database. `openDatabase` calls `verifyRecordedSchemaVersion` first, which refuses that exact state with `storage_unavailable/verify-user-version`. The target now models the real open sequence, which turns the finding into proof the guard is load-bearing.

**On-disk migration cost, measured and kept out of the loop honestly.** A file-backed full migration run costs 60–213 ms here against ~2.5 ms in memory, essentially all fsync, and file-backed iterations produced budget failures at iterations 0 and 250. Rather than raising the budget (blinding the target to a real hang) or dropping the case (leaving WAL and fsync unproven), the on-disk path runs as a bounded timed pre-check at `user_version` 0, 3, 11, 12 and 255 with its cost reported in the run output.
